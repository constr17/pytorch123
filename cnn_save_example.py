# How to save and load models in Pytorch (https://youtu.be/g6kQl_EFn84?si=LNy35YzpX2OtTBmk)
...
def save_checkpoint(checkpoint, filename='my_checkpoint.pth.tar'):  # Save model and optimizer state
    print('Saving checkpoint...')
    torch.save(checkpoint, filename) # Save checkpoint


def load_checkpoint(filename='my_checkpoint.pth.tar'):  # Load model and optimizer state
    print('Loading checkpoint...')
    check_point = torch.load(filename)  # Load checkpoint
    model.load_state_dict(check_point['state_dict'])  # Load model state
    optimizer.load_state_dict(check_point['optimizer'])  # Load optimizer state
...
if load_model: # Load model
    load_checkpoint(filename='my_checkpoint.pth.tar') # Load model and optimizer state

for epoch in range(num_epochs): # Training loop
    losses = []
    start = time.time()

    if epoch % 3 == 0: # Save model and optimizer state
        checkpoint = {'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict()} # Save model and optimizer state
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        losses.append(loss.item()) # Add loss to list
